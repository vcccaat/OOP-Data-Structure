...
bool Buddy::capable(Activity<Workout> workout_activity)
{
	 ...
	 * Add your code here!
	 * */
	if (getEnergy() + workout_activity.item.getEnergyChange() >= 0 &&
		getMuscle() + workout_activity.item.getMuscleChange() >= 0 &&
		getFat() + workout_activity.item.getFatChange() >= 0)
	{
		if (workout_schedule.isFree(workout_activity.start_time, workout_activity.end_time))
		{
			return true;
		}
	}
...
bool Buddy::capable(Activity<Dining> dining_activity) {
     ...
     * Add your code here!
     * */
	if (getMoney() - dining_activity.item.getPrice() >= 0)
	{
